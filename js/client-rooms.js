(function ($) {
	...
	this.RoomsRoom = Room.extend({
		...
		renderRoomBtn: function (roomData) {
			...
			var buf = '<div><a href="AX-Showdown.html#' + id + '" class="ilink"><small style="float:right">(' + Number(roomData.userCount) + ' users)</small><strong><i class="fa fa-comment-o"></i> ' + BattleLog.escapeHTML(roomData.title) + '<br /></strong><small>' + BattleLog.escapeHTML(roomData.desc || '') + '</small></a>';
			if (roomData.subRooms && roomData.subRooms.length) {
				...
				for (var i = 0; i < roomData.subRooms.length; i++) {
					buf += ' <a class="ilink" href="AX-Showdown.html#' + toID(roomData.subRooms[i]) + '"><i class="fa fa-comment-o"></i> <strong>' + BattleLog.escapeHTML(roomData.subRooms[i]) + '</strong></a>';
				}
				buf += '</div>';
			}
			buf += '</div>';
	...
	this.BattlesRoom = Room.extend({
		...
		renderRoomBtn: function (id, roomData, matches) {
			...
			return '<div><a href="AX-Showdown.html#' + id + '" class="ilink">' + roomDesc + '</a></div>';
		},
